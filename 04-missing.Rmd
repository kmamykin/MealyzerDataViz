...
```{r}
users.selected = users.all %>%
  filter(complete_meals > 14)
print(users.selected[["user_id"]])
```
...
meals.imputed = meals.all %>%
  ...
  filter(premeal_bg_delay_minutes >= 0 & postmeal_bg_delay_minutes >= 0 ) # keep only positive delays
```
...
gridExtra::grid.arrange(
  g1, g2, widths = c(2, 6), heights = c(1,1),
