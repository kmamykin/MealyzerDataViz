...
Below are some of the visualizations selected to highlight some point about the data visualized. Not all visualizations we tried were insightful or had any talking points and therefor not included in this report main results section.
...
meals.selected %>% 
  ...
  # draw rect purely so that the legend appears as a solid rect, for a line with low alpha which makes it hard to discern
  ...
  geom_hline(aes(yintercept=180), color="red", show.legend = FALSE, alpha = 0.5) +
...
Interestingly, most of users manage diabetes well, have a flat response to meals and barely exceeding the warning line of 180 mg/dL. Some users (namely 2254, 1809, 2262) have big ups and downs, and in case of user 1809 BG stays above the 180 level all the time.
...
## Trends in calories and nutrients
...
meals.selected %>%
  ...
  facet_wrap(~user_fct, ncol=6) +
  labs (
    title = "Trends of total calories consumed by user",
    x = "Time",
    y = "Calories"
  ) + 
  theme(
...
meals.selected %>%
  ...
  facet_wrap(~user_fct, ncol=6) +
  labs (
    title = "Trends of fasting BG by user",
    x = "Time",
    y = "Fasting BG"
  ) + 
  theme(
